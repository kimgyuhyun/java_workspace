...
public class TestInstanceofExample {
	public static void method1(TestParent parent) { //클래스타입 매개변수 객체를 다룰때 많이 사용함
		if(parent instanceof TestChild) { //매개변수 parent 가 어느 객체를 참조하고있는지 확인하고 TestChild 면 자동타입변환된 상태니 true
			TestChild child = (TestChild) parent; //조건식이 true 면 TestChild로 강제타입변환
	...
	public static void method2(TestParent parent) {
		TestChild child = (TestChild) parent; //객체 타입 확인을 안해서 classCastException이 발생할 가능성이 있음
		System.out.println("method2 - TestChild로 변환 성공");
	}
	
	public static void main(String[] args) {
		TestParent parentA = new TestChild(); //TestChild 객체를 TestParent 타입으로 참조
		...
		TestParent parentB = new TestParent(); //TestParent 객체 생성
